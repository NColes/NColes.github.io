...
function generateCustomReleaseTable() {
  ...
  for (var i = 0; i < JSON.result.results.length; i++) {
    ...
    if (new RegExp(dashboardData.join("|"), "i").test(JSON.result.results[i].description.title) && new RegExp(trackingTable.join("|"), "i").test(JSON.result.results[i].description.title)) {
      ...
      DB = '<img src="http://ncoles.github.io/releasesheet/dashboard.svg" class="DB" data-placement="left" title="This release feeds into the UK post-referendum economy dashboard." style="margin-left:2px;"><div style="display:none;">DB</div>';
    } else if (new RegExp(trackingTable.join("|"), "i").test(JSON.result.results[i].description.title)) {
      DB = '<img src="http://ncoles.github.io/releasesheet/dashboard.svg" class="DB" data-placement="left" title="This release feeds into the &ldquo;Tracking the impact of the EU referendum&rdquo; table." style="margin-left:2px;"><div style="display:none;">TT</div>';
    };
    ...
    if (JSON.result.results[i].description.cancelled == true) {
      ...
      cancelled = '<img src="http://ncoles.github.io/releasesheet/cancelled.svg" class="cancelled" data-placement="left" data-placement="right" title="Cancelled: ' + cancelledReason + '"><div style="display:none;">Cancelled</div>';
    };
